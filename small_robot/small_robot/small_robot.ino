...
void forward();  // sets forward and go forward //checked   //int Left = Left_MotorBase_speed, int Right = Right_MotorBase_speed
void pid_forward(int steps);  // go forward with pid output for a given number of steps //checked
void backward();//checked    //int Left = Left_MotorBase_speed, int Right = Right_MotorBase_speed
void pid_backward(int steps);//not checked, not working( need to change back line following algo)
void turn_left_90();//checked   //int dTime = dL90
void turn_right_90();//checked   //int dTime = dR90
void turn_left_180();//not checked   //int dTime = dL180
void stop();//checked
...
int read_color_sensor(){
  ...
  do{ 
    ...
    else if ((redfrequency>130 && redfrequency <240) && (bluefrequency>40 && bluefrequency<110) && (greenfrequency>90 && greenfrequency<190)) B_val++;
...
void forward(){ // left right speeds   //int Left=Left_MotorBase_speed, int Right=Right_MotorBase_speed
    set_forward();
...
void backward(){  //int Left=Left_MotorBase_speed, int Right=Right_MotorBase_speed
...
void turn_left(){  //int left = Left_MotorBase_speed , int right = Right_MotorBase_speed
    ...
    digitalWrite(IN4, HIGH);
    LMotorSpeed = Left_MotorBase_speed; // may need to change this
    RMotorSpeed = Right_MotorBase_speed;
    motor_speed();
}
...
void turn_left_90(){
    ...
    do {
        read_ir();
    } while (!(IR_Bin_val[3] == 0 && IR_Bin_val[4] == 0)); // && IR_val[0] == 0 && IR_val[1] == 0 && IR_val[6] == 0 && IR_val[7] == 0);
        stop();
}
...
void checkpoint_one_to_gate_A()
{
    if (stage==0)
    {
      current_time=millis();
        ...
        {
            read_ir();
            if (IR_Bin_val[7]==0 && IR_Bin_val[6]==0 && IR_Bin_val[5]==0)
            {
...
void vault_to_ramp_junction()
{
    ...
    {
        forward();
        while (!(IR_val[0]==0 && IR_val[1]==0 && IR_val[2]==0))
        {
            line_follow();
        }
            stop();
